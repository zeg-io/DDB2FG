...
let startXML = '<?xml version="1.0" encoding="iso-8859-1"?>\n'
...
let abilSTR = [0, 0, 0, 0, 0]
let abilCHA = [0, 0, 0, 0, 0]
let abilCON = [0, 0, 0, 0, 0]
let abilDEX = [0, 0, 0, 0, 0]
let abilINT = [0, 0, 0, 0, 0]
let abilWIS = [0, 0, 0, 0, 0]
...
$(function () {
  ...
  let uploadObj = $('#fileuploader').uploadFile({
    url: 'upload.php',
    multiple: false,
    dragDrop: true,
    maxFileCount: 1,
...
function buildOutput () {
  ...
  for (m = 0; m < allTraits.length; m++) {
    ...
    buildXML += '\t\t\t\t<source type="string">' + convertCase(popCharRace) + '</source>\n'
  ...
  for (n = 0; n < allFeatures.length; n++) {
    ...
    buildXML += '\t\t\t\t<source type="string">' + convertCase(charBG) + '</source>\n'
    buildXML += '\t\t\t</id-' + thisIteration + '>\n'
  }
  buildXML += '\t\t</featurelist>\n'
...
const replaceDash = str => {
  let firstStep = str.replace(/-/g, '_')
  
  return firstStep.replace(/ /g, '_')
}

const removeEntry = (array, element) => {
